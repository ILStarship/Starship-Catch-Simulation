try:
    ...
    class Starship:
        ...
        def fire_engine(self, thrust, propellant_usage):
            """Err... Fires engine"""
            global descent_rate, current_audio
            ...
            if self.propellant > 0:
                ...
                self.firing = True
                current_audio = AUDIO.ENGINESOUND
                if not pygame.mixer.music.get_busy() or current_audio == AUDIO.ENGINESOUND:
                    pygame.mixer.music.load(engine_sound)
                    pygame.mixer.music.play()
                    print("Playing engine")
            else:
                # Propellant used up
                self.img = starship_img # Set image to image of starship not firing engines
                self.firing = False
                if pygame.mixer.music.get_busy() and current_audio == AUDIO.ENGINESOUND:
                    pygame.mixer.music.stop()
                    current_audio = AUDIO.NOAUDIO
                    "Stopping playing engine"
        ...
        def explode(self):
            ...
            pygame.mixer.music.play()
            current_audio = AUDIO.EXPLOSIONSOUND
    ...
    class AUDIO(Enum):
        """Keeps track of which audio file pygame.mixer is currently playing"""
        NOAUDIO = 1
        ENGINESOUND = 2
        EXPLOSIONSOUND = 3
        FALLINGSOUND = 4
    
    current_audio = AUDIO.NOAUDIO # Keeps track of what audio pygame.mixer is currently playing
...
while running:
    try:
        ...
        #        while running:
            ...
            # Play sound of ship falling through air
            if current_audio != AUDIO.ENGINESOUND and current_audio != AUDIO.EXPLOSIONSOUND and not pygame.mixer.music.get_busy():
                # No other audio playing
                print(f"Playing wind. current_audio: {AUDIO(current_audio)}, pygame.get_busy(): {pygame.mixer_music.get_busy()}")
                #print(current_audio)
                pygame.mixer.music.load(wind_sound)
                pygame.mixer.music.play()
                current_audio = AUDIO.FALLINGSOUND
